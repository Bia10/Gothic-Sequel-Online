...
void CChat::Render()
{
	if(this->IsInitiated() == true && this->IsShowing() == true)
	{
		...
		if( /*game->GetShowPlayerStatus()*/ true ) //Ukrywanie wiadomoœci na czacie gdy menu jest aktywne
		{
		...
		{
			lineDistance = (150 * lines.size()) + 400;
			...
			//Gestykulowanie
			if( this->timeGesticulation < GetTickCount() )
			{
				...
				this->timeGesticulation = GetTickCount() + 3500; //zauwa¿y³em ¿e zwyk³a animka gestykulacji trwa 3.5 sekundy
			}
		}
		//Stara czcionka
		//screen->SetFont(font);
...
void CChat::KeyEvent(int key)
{
	if(key == KEY_T && this->IsInputActive() == false)
	{
		...
		this->lastLetter = "t";
		this->timeLastKey = GetTickCount() + 175; //175ms, bo ostatni klawisz to "t", a nie chcemy zeby przy kazdym otwarciu czatu sie drukowal
		oCNpc* hero = oCNpc::GetHero();
		if( hero )
		{
	...
	else if(this->IsInputActive() == true)
	{
		...
		else
		{
			...
			if( c_key )
			{
				...
				{
					if( GetTickCount() > this->timeLastKey )
					{
						this->currentText += c_key;	
						this->timeLastKey = GetTickCount() + 160;
					}
				}
				else
				{
					...
					this->timeLastKey = GetTickCount() + 130;
				}
			}
		}
	}
