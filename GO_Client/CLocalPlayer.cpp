...
void CLocalPlayer::SendBasicSync()
{
	if( basicSyncTimer < GetTickCount() )
	{
		basicSyncTimer = GetTickCount() + 200;
		CNetwork* net = core.GetNetwork();
		if(net->GetPeer()->GetLastPing(net->GetServerAddress()) > PING_SYNC_LIMIT) return;
		oCNpc* hero = oCNpc::GetHero();
		zVEC3 pos = hero->GetPosition();
		float angle = hero->GetHeading();
		zCModelAni* anim = hero->GetCurrentAni();
		if(anim==NULL) return;
		RakString animation = anim->GetAniName().ToChar();
		BitStream bs;
		bs.Write((MessageID)GO_PLAYER);
		bs.Write((MessageID)BASIC_SYNC);
		bs.Write(pos.x);
		bs.Write(pos.y);
		bs.Write(pos.z);
		bs.Write(angle);
		...
		net->GetPeer()->Send(&bs, LOW_PRIORITY, UNRELIABLE, 0, net->GetServerAddress(), false);
	}
}
void CLocalPlayer::SendChangeWeaponMode()
{
	if( weaponModeBroadcastTimer < GetTickCount() )
	{
		...
		weaponModeBroadcastTimer = GetTickCount() + 300;
	}
};

void CLocalPlayer::SendWearArmor()
{
	if( armorBroadcastTimer < GetTickCount() )
	{
		oCNpc* hero = oCNpc::GetHero();
		if( hero )
		{	
		...
		armorBroadcastTimer = GetTickCount() + 1000;
	}
};

void CLocalPlayer::SendEquipWeapon()
{
	if( weaponBroadcastTimer < GetTickCount() )
	{
		oCNpc* hero = oCNpc::GetHero();
		if( hero )
		{
		...
		weaponBroadcastTimer = GetTickCount() + 1000;
	}
};

void CLocalPlayer::SendInstanceChange()
{
	if( instanceBroadcastTimer < GetTickCount() )
	{
		oCNpc* hero = oCNpc::GetHero();
		if( hero )
		{
			...
			if( strcmp(instance.ToChar(), this->instance.C_String()) != 0 )
			{
				...
				instanceBroadcastTimer = GetTickCount() + 3000; //co 5 sekund xd | OK, ale lepiej co 3 xD
			}
		}
	}
};
...
{
	if( this->handBroadcastTimer < GetTickCount() )
	{
		oCNpc* hero = oCNpc::GetHero();
		if( hero )
		{
		...
		this->handBroadcastTimer = GetTickCount() + 700;
	}
};
...
{
	if( this->standUpTimer < GetTickCount() )
	{
		oCNpc* hero = oCNpc::GetHero();
		if( hero )
		{
		...
		this->standUpTimer = GetTickCount() + 1000;
	}
};
...
{
	if (this->protectionBroadcastTimer < GetTickCount())
	{
		oCNpc *hero = oCNpc::GetHero();
		...
		this->protectionBroadcastTimer = GetTickCount() + 1200;
	}
};
*/
...
{
	if( this->worldBroadcastTimer < GetTickCount() )
	{
		...
		this->worldBroadcastTimer = GetTickCount() + 800;
	}
};
...
void CLocalPlayer::Respawn()
{
	...
	if( lastDeathTimer < GetTickCount() )
	{
		oCNpc* hero = oCNpc::GetHero();
		if( hero->IsDead() )
		{
			if( isDead == false )
			{
				lastDeathTimer = GetTickCount() + 5000;
				isDead = true;
			...
			isDead = false;
		lastDeathTimer = GetTickCount() + 300;
	}
