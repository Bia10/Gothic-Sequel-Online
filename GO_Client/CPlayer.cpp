...
void CPlayer::RefreshPositions()
{
	...
	else if( GetTickCount() > timerSlide )
	{
		if( isSlideAllowed == false )
		{
			isSlideAllowed = true;
			timerSlide = GetTickCount() + 1000; //Œlizg
		}
		else
		{
			isSlideAllowed = false;
			timerSlide = GetTickCount() + 300; //Przerwa
		}
	}
};
...
void CPlayer::RefreshArmor()
{
	if( GetTickCount() > timerArmor )
	{
		...
		timerArmor = GetTickCount() + 3000;
	}
};

void CPlayer::RefreshHealth()
{
	if( GetTickCount() > timerHealth )
	{
		if( health > 0 )
		{
		...
		timerHealth = GetTickCount() + 1200;
	}
};

void CPlayer::RefreshWeapon()
{
	if( GetTickCount() > timerWeapon )
	{
