...
void PlayerRPC::BasicSync(BitStream& s)
{
	//DLOG("PlayerRPC::BasicSync()");
	int playerID;
	s.Read(playerID);
	if( playerID != core.GetMultiplayer()->GetMyID() )
	{
		...
		if(player != NULL)
		{
			...
			if(player->npc != NULL)
			{
				if(player->npc->GetAttribute(NPC_ATR_HITPOINTS) > 0 && player->npc->IsUnconscious() == 0)
				{
					...
					if(model != NULL)
						model->StartAni(zSTRING(player->animation.C_String()), 0);
				}
			}
		}
	}
		...
		if (tree->data)
		{
			...
			if (goMath::GetDistance3D(x, y, z, pos.x, pos.y, pos.z) < 10)
			{
				...
				case VOB_TYPE_VT_OCMOBDOOR:
					...
					core.GetMultiplayer()->triggerQueue.push(STriggerSync(GetTickCount() + 1200, triggerNpc, mob));
					break;
