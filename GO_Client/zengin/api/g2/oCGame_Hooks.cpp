...
void oCGame::Hook_Render()
{
	...
	pMemLib->ImportHook(0x006648F0, sizeof(void(oCGame::*)(void)), &oCGame::Hook_Render);
};

int oCGame::Hook_HandleEvent(int key)
{
	pMemLib->RemoveHook(0x0068A300);
	int result = 1;
	...
	pMemLib->ImportHook(0x0068A300, sizeof(int(oCGame::*)(int)), &oCGame::Hook_HandleEvent);
...
void oCGame::Hook_ChangeLevel(zSTRING const & map1, const zSTRING & map2)
{
	...
	pMemLib->ImportHook(0x00663950, sizeof(void(oCGame::*)(const zSTRING&, const zSTRING&)), &oCGame::Hook_ChangeLevel);
};

void oCGame::Hook_EnterWorld(oCNpc *npc, int i, const zSTRING & str)
{
	pMemLib->RemoveHook(0x00665950);
	...
	pMemLib->ImportHook(0x00665950, sizeof(void(oCGame::*)(oCNpc*,int,const zSTRING&)), &oCGame::Hook_EnterWorld);
};
...
void oCGame::Fake_LoadSavegame(int a, int b)
{
	...
	if (scr.GetScriptVars()->isSavingActive)
	{
		...
		if( core.IsLateHooksInitiated() == false )
			core.InitializeLateHooks();
		pMemLib->ImportHook(0x00662D60, sizeof(void(oCGame::*)(int,int)), &oCGame::Fake_LoadSavegame); 
	}
#endif //COOP
};
...
void oCGame::Fake_WriteSavegame(int a, int b)
{
	...
	if (scr.GetScriptVars()->isSavingActive)
	{
		...
		if( core.IsLateHooksInitiated() == false )
			core.InitializeLateHooks();
		pMemLib->ImportHook(0x00661680, sizeof(void(oCGame::*)(int,int)), &oCGame::Fake_WriteSavegame);
	}
#endif //COOP
};