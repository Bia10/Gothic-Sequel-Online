...
void oCMobInter::Hook_OnTrigger(zCVob *target, zCVob *sender)
{
	...
	pMemLib->ImportHook(0x006ABF70, sizeof(void(oCMobInter::*)(zCVob*, zCVob*)), &oCMobInter::Hook_OnTrigger);
}

void oCMobInter::Hook_OnUntrigger(zCVob *target, zCVob *sender)
{
	pMemLib->RemoveHook(0x006AC2A0);
	/*if (!this->GetS1() && this->GetS2())
	{
		if (oCNpc::GetHero() == (oCNpc*)sender)
		{
	...
	pMemLib->ImportHook(0x006AC2A0, sizeof(void(oCMobInter::*)(zCVob*, zCVob*)), &oCMobInter::Hook_OnUntrigger);
}