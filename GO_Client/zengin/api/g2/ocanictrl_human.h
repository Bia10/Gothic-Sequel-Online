...
{
public:
	static zCAICamera* GetCurrent() {XCALL(0x004A59B0);};
	void SetVob(zCVob* vob){XCALL(0x004A6A00);};
	...
	zCVob* GetVob() const {return *(zCVob**)((DWORD)this+0x98);}; //??? FROM G1!
	void SetTarget(zCVob* target){XCALL(0x004A6CE0);};
...
private:
	...
	int dummyLastVar;
	public:
		void ShowWeaponTrail(){ XCALL(0x0064C360);};
		void CreateHit(zCVob* vob){ XCALL(0x0064CCF0);};
		void HitInterrupt() { XCALL(0x0064D1A0); };
		int IsWalking(){ XCALL(0x0064A540);};
		int GetActionMode() { XCALL(0x007874C0);};
		void _Stand() { XCALL(0x006534C0);};
		void StopTurnAnis() { XCALL(0x0064A7C0);};
};
...
public:
	int MagicInvestSpell() {XCALL(0x004746E0);};
	int MagicCheckSpellStates(int) {XCALL(0x00474D60);};
	void SetCamMode(const zSTRING& mode, int p2 = 0){XCALL(0x00632AD0);};
	void ChangeCamMode(const zSTRING& mode){XCALL(0x006326C0);};
};
