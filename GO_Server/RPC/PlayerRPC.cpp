...
void PlayerRPC::BasicSync(Packet* packet, BitStream& s)
{
	...
	if(player && player->bConnected == true && player->spawned == true)
	{
		...
		s.Read(player->z);
		s.Read(player->angle);
		s.Read(player->animation);
		
		BitStream bs;
		bs.Write((MessageID)GO_PLAYER);
		bs.Write((MessageID)BASIC_SYNC);
		bs.Write(player->playerId);
		bs.Write(player->x);
		bs.Write(player->y);
		bs.Write(player->z);
		bs.Write(player->angle);
		bs.Write(player->animation);
		core.GetNetwork()->SendToPlayersOnList(bs, LOW_PRIORITY, UNRELIABLE, &player->streamedPlayers);
	}
}
